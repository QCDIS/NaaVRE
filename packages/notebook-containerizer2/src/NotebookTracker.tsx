...
import { requestAPI, CellPreview as NotebookPreview, VRECell } from '@jupyter_vre/core';
...
import { Button, FormControl, MenuItem, Select, TableBody,  ThemeProvider, TextField } from "@material-ui/core";
...
const AddNotebookDialogOptions: Partial<Dialog.IOptions<any>> = {
    title: '',
...
export class NotebookTracker extends React.Component<IProps, IState> {

    state = DefaultState;
    notebookPreviewRef: React.RefObject<NotebookPreview>;

    constructor(props: IProps) {
        super(props);
        this.notebookPreviewRef = React.createRef();
    }

    handleCreateNotebook = async () => {
        showDialog(AddNotebookDialogOptions)
    }
    ...
    exctractor = async (notebookModel: INotebookModel, save = false) => {
        // try {
            const extractedCell = await requestAPI<any>('nbcontainerizer/extract', {
                body: JSON.stringify({
            ...
            this.notebookPreviewRef.current.updateChart(extractedCell['chart_obj']);
        // } catch (error) {
        //     console.log(error);
            ...
            <ThemeProvider theme={theme}>
                <div>
                    <div className={'lw-panel-editor'}>
                        <NotebookPreview ref={this.notebookPreviewRef} />
                    </div>
                    {this.props.notebook != null ? (
                        <div>
